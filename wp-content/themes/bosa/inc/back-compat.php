 ...
 *
 * Prevents Bosa from running on WordPress versions prior to 5.0,
 ...
 * Bosa on WordPress versions prior to 5.0.
 *
 * @since Bosa 1.0.0
 * @global string $wp_version WordPress version.
 */
function bosa_upgrade_notice() {
	$message = sprintf( esc_html__( 'Bosa requires at least WordPress version 5.0. You are running version %s. Please upgrade and try again.', 'bosa' ), $GLOBALS['wp_version'] );
...
function bosa_customize() {
	wp_die( sprintf( esc_html__( 'Bosa requires at least WordPress version 5.0. You are running version %s. Please upgrade and try again.', 'bosa' ), $GLOBALS['wp_version'] ), '', array(
		'back_link' => true,
	) );
}
...
function bosa_preview() {
	if ( isset( $_GET['preview'] ) ) {
		wp_die( sprintf( esc_html__( 'Bosa requires at least WordPress version 5.0. You are running version %s. Please upgrade and try again.', 'bosa' ), $GLOBALS['wp_version'] ) );
	}
}
